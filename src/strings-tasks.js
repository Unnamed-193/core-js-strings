...
function extractNameFromTemplate(value) {
  let nameStr = value.replace('Hello, ', '');
...
function unbracketTag(str) {
  let removeAngle = str.replace('<', '');

  removeAngle = removeAngle.replace('>', '');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < chars.length; i += 1) {
    ...
    if (upper.includes(currentChar)) {
      const index = upper.indexOf(currentChar);

      const encodedIndex = (index + 13) % 26;

      encodedChar = upper[encodedIndex];
    }

    if (lower.includes(currentChar)) {
      const index = lower.indexOf(currentChar);

      const encodedIndex = (index + 13) % 26;

      encodedChar = lower[encodedIndex];
    }
